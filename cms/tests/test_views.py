...
class ViewTests(CMSTestCase):
    ...
    def test_page_language_from_request(self):
        """
        Test that page content respects request.LANGUAGE_CODE when rendering,
        regardless of whether language prefix patterns are used.
        """
        # Create a page with content in multiple languages
        page = create_page("Test", "nav_playground.html", "en")
        create_page_content("de", "Test DE", page)
        create_page_content("fr", "Test FR", page)
        ...
        response = details(request, page.get_path("fr"))

        # Verify the response contains French content
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context_data.get("lang"), "fr")

        # Test with a different language
        request.LANGUAGE_CODE = "de"
        response = details(request, page.get_path("de"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context_data.get("lang"), "de")
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context_data.get("lang"), "en")
